...
runNonExonApp() # Pop out the shiny page
```
![Figure 1: The example shiny page (a screenshot )](../inst/extdata/testdata/example_output.png "example output"){width=90%}
...
### Input for shiny app (after run `runNonExonApp`)
  ...
  In shiny app, there is one more input, the reference sequence. This allows users to choose which transcript seqeuence they would like to know more about if there are more than one transcript seqeunces in trancripts file. 
...
1. **Reference Sequence** is the first transcript sequence in transcripts file.
